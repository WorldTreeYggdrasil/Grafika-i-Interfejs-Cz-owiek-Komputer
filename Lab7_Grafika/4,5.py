...
def render(time):
    ...
    rotation_matrix1 = glm.rotate(glm.mat4(1.0), glm.radians(time*45), glm.vec3(0.0, 0.0, 1.0))
    rotation_matrix2 = glm.rotate(glm.mat4(1.0), glm.radians(time*45), glm.vec3(1.0, 0.0, 0.0))  # New rotation axis
    rotation_matrix = rotation_matrix1 * rotation_matrix2

    # Adjust the camera position and target
    V_matrix = glm.lookAt(
        glm.vec3(5.0, 5.0, 10.0),  # Move the camera to the middle of the array and 10 units away along the Z-axis
        glm.vec3(5.0, 5.0, 0.0),  # Looking at the middle of the array
